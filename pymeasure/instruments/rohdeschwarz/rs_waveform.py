...
class GenericTag(object):
    ...
    def to_file_text(self, field):
        """ Translate field to text

        Return a value suitable for string representation in file format using
...
.. warning:: When the tag name is WWAVEFORM (double W) instead of WAVEFORM,
     this is probably some proprietary encrypted format from Rohde & Schwarz
     and so it is not possible to correctly decode the content.

    """
    pattern = r"{(?P<name>W?WAVEFORM)-(?P<size>\d+):(?P<extra_chars>0,)?#"
...
class EmptyTag(BinaryTag):
    ...
    pattern = r"{(?P<name>EMPTYTAG)-(?P<size>\d+):#"
...
class IndexTag(GenericTag):
    pattern = r"{(?P<name>(?:[^}]*?) LIST) (?P<index>\d+):(?P<listvalue>.*?)}"
...
class IntegerTag(GenericTag):
    pattern = r"{(?P<name>CLOCK|SAMPLES|MWV_SEGMENT_COUNT):(?P<value>\d+)}"
...
class SegmentTag(GenericTag):
    pattern = r'{(?P<name>MWV_SEGMENT_(?:LENGTH|START|CLOCK|LEVEL_OFFS|FILES))' + \
        r':(?P<listvalue>(?:.*?)(?:(?:,.*?)*))}'
...
class SegmentClockModeTag(GenericTag):
    pattern = r"{(?P<name>MWV_SEGMENT_CLOCK_MODE):\s*?(?P<value>UNCHANGED|HIGHEST|USER)}"


class SegmentCommentTag(GenericTag):
    pattern = r"{(?P<name>MWM_SEGMENT(?P<index>\d+)_COMMENT):(?P<value>.*?)}"


class CLW4Tag(BinaryTag):
    pattern = r"{?P<name>CONTROL LIST WIDTH4-(\d+):#"
...
if __name__ == "__main__":
    ...
    parser.add_argument("--info",
                        help="print infomation details for the input file",
